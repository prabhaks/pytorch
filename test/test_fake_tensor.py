...
class FakeTensorTest(TestCase):
    ...
    def checkType(self, t, device_str, size):
        self.assertTrue(is_fake_leaf(t))
    ...
    def test_basic(self):
        ...
        with FakeTensorMode() as mode:
            ...
            self.assertTrue(is_fake_leaf(z))
    ...
    def test_shape_take_not_device(self):
        with FakeTensorMode():
            ...
            self.assertTrue(is_fake_leaf(out))
    ...
    def test_constructor(self):
        ...
        self.assertTrue(is_fake_leaf(x))
    ...
    def test_fake_grad_copy(self):
        ...
        self.assertTrue(is_fake_leaf(fake_x.grad))
    ...
    def test_like_constructor(self):
        with FakeTensorMode():
            x = torch.rand([4, 4])
            y = torch.ones_like(x)
            self.assertTrue(is_fake_leaf(y))
        ...
        for i, context in enumerate([contextlib.nullcontext, lambda: mode]):
            with context():
                ...
                for inps in [inps1, inps2]:
                    ...
                    for ten in out:
                        if i == 1:
                            self.assertTrue(is_fake_leaf(ten))
    ...
    def test_deepcopy(self):
        ...
        def check_copy(mod, mod_copied):
            for name, param in itertools.chain(
                ...
                self.assertTrue(is_fake_leaf(param_copied))
                self.assertEqual(
...
[torch.storage.TypedStorage(dtype=torch.float32, device=meta) of size 3]""",
    ...
    def test_move_module_under_fake(self):
        ...
        for p in m.parameters():
            self.assertTrue(is_fake_leaf(p))
            self.assertEqual(p.device, gpu_device)
        for b in m.buffers():
            self.assertTrue(is_fake_leaf(b))
    ...
    def test_fake_tensor_prop_on_nn_module(self):
        ...
        with FakeTensorMode() as fake_tensor_mode:
            ...
            def to_fake_tensor(x):
                if isinstance(x, torch.Tensor) and not is_fake_leaf(x):
            ...
            with torch.nn.utils.stateless._reparametrize_module(
                ...
                self.assertTrue(is_fake_leaf(result))
...
class FakeTensorDispatchCache(TestCase):
    ...
    def test_meta_tensor_to_fake_cpu(self):
        ...
        self.assertTrue(is_fake_leaf(x_cpu))
...
class FakeTensorMetaDevicePropagation(TestCase):
    ...
    def test_inplace_add_with_meta_rhs_keeps_destination_device(self, device):
        ...
        with FakeTensorMode():
            ...
            self.assertTrue(is_fake_leaf(log_det))
